...
class ShopListDal(ABCShopListDal):
    ...
    def get_shop_lists_from_user(self, user_id) -> List[ShopList]:
        ...
        return (
            self.db.query(ShopList).filter(ShopList.shop_list_id.in_(shop_list_ids)).all()
        )

    def get_shop_list_by_id(self, shop_list_id: int) -> ShopList:
        return (
            self.db.query(ShopList).filter(ShopList.shop_list_id == shop_list_id).first()
        )
    ...
    def delete_shop_list(self, shop_list_id: int):
        ...
        self.db.query(Item).filter(Item.shop_list_id == shop_list_id).delete()
        self.db.query(UserList).filter(UserList.shop_list_id == shop_list_id).delete()
        self.db.query(ShopList).filter(ShopList.shop_list_id == shop_list_id).delete()

...
class UserListDal(ABCUserListDal):
    ...
    def get_user_list_by_user_id(self, shop_list_id: int, user_id: int) -> UserList:
        user_list = (
            ...
            .filter(
                and_(UserList.shop_list_id == shop_list_id, UserList.user_id == user_id)
            )
            .first()
        )
        return user_list
    ...
    def get_user_list_by_user_list_id(self, user_list_id: int) -> UserList:
        user_list = (
            self.db.query(UserList).filter(UserList.user_list_id == user_list_id).first()
        )
        return user_list

    def get_user_lists_by_shop_list_id(self, shop_list_id: int) -> List[UserList]:
        user_lists = (
            self.db.query(UserList).filter(UserList.shop_list_id == shop_list_id).all()
        )
        return user_lists
    ...
    def update_user_list(self, user_list: UserList) -> UserList:
        self.db.query(UserList).filter(
            ...
            {"is_adm": user_list.is_adm, "is_nutritionist": user_list.is_nutritionist}
        )
        self.db.commit()
    ...
    def delete_user_list(self, user_list_id: int) -> UserList:
        ...
        self.db.query(UserList).filter(UserList.user_list_id == user_list_id).delete()

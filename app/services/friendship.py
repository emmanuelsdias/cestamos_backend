...
class FriendshipService(ABCFriendshipService):
    ...
    def construct_friendship_dto(
        self, user: UserModel, friendship: FriendshipModel
    ) -> Friendship:
    ...
    def get_friendships(self, token) -> List[Friendship]:
        ...
        friendships_res = [
            self.construct_friendship_dto(user, friendship) for friendship in friendships
        ]
    ...
    def delete_friendship(self, friendship_id: int, token: str) -> Friendship:
        ...
        if current_friendship is None:
            raise HTTPException(status_code=400, detail="Friendship doesn't exist")

...
class ABCInvitationService(UserBasedService):
    ...
    def delete_invitation(
        self, invitation_id: int, token: str, accepted: bool
    ) -> Invitation:
        """Delete a invitation"""
...
class InvitationService(ABCInvitationService):
    ...
    def check_invitation_validity(self, invitation_id, user):
        ...
        if invitation is None:
            raise HTTPException(status_code=400, detail="Invitation doesn't exist")
    ...
    def get_invitations(self, token) -> List[Invitation]:
        ...
        invitations_res = [
            self.construct_invitation_dto(invitation) for invitation in invitations
        ]
    ...
    def create_invitation(self, invitation: InvitationCreate, token: str) -> Invitation:
        ...
        if invited_user is None:
            raise HTTPException(status_code=400, detail="Invited user doesn't exist")
        if user.user_id == invitation.user_id:
            raise HTTPException(status_code=400, detail="Can't invite yourself")
        if (
            self.dal.get_invitation_from_pair(user.user_id, invitation.user_id)
            is not None
        ):
            raise HTTPException(status_code=400, detail="Invitation already exists")
        if (
            self.friendship_dal.get_friendship_from_user_pair(
                user.user_id, invited_user.user_id
            )
            is not None
        ):
        ...
        db_invitation = InvitationModel(
            user_id1=user.user_id, user_id2=invitation.user_id
        )
    ...
    def delete_invitation(
        ...
        if user.user_id == invitation.user_id2:
            if accepted is None:
                raise HTTPException(status_code=400, detail="missing accepted flag")
            if accepted:
                new_friendship = FriendshipModel(
                    user_id1=invitation.user_id1, user_id2=invitation.user_id2
                )
